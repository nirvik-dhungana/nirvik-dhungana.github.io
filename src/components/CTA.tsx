...
import { Github, Linkedin, Twitter, ArrowRight, Gitlab, Code2, Terminal } from 'lucide-react';
...
export function CTA() {
  return (
    <section id="contact" className="relative w-full bg-bg-1 pt-[96px] lg:pt-[140px] pb-12 px-6 overflow-hidden border-t border-bg-3/40">
      {/* Ambient Blob */}
      <motion.div
        animate={{
          scale: [1, 1.1, 1],
      ...
      >
        ...
        <motion.h2 variants={itemVariants} className="text-5xl md:text-7xl font-display font-bold text-fg-bright mb-6 tracking-tight">
          {CTAContent.heading}
        </motion.h2>

        <motion.p variants={itemVariants} className="text-xl md:text-2xl text-fg-dim mb-12 max-w-2xl">
          {CTAContent.subtext}
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="mb-24">
          ...
          >
            ...
            >
              <ArrowRight size={20} className="transition-transform duration-300 group-hover:translate-x-1.5" strokeWidth={2.5} />
            </motion.span>
          </a>
        </motion.div>

        {/* Footer info */}
        <motion.div variants={itemVariants} className="w-full flex flex-col items-center gap-6 pt-12 border-t border-bg-3/40">
          <div className="flex flex-wrap justify-center items-center gap-8">
            {[
              ...
              { icon: Twitter, href: PersonalInfo.twitter, label: 'Twitter Profile' },
              { icon: Gitlab, href: PersonalInfo.gitlab, label: 'GitLab Profile' },
              { icon: Code2, href: PersonalInfo.leetcode, label: 'LeetCode Profile' },
              { icon: Terminal, href: PersonalInfo.freecodecamp, label: 'freeCodeCamp Profile' },
            ].map((social, idx) => (
              <a
                key={idx}
                href={social.href}
