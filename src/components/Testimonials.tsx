...
export function Testimonials() {
  ...
  useEffect(() => {
    ...
    const items = Array.from(container.children).filter(child => child.tagName !== 'STYLE');
  ...
  const scroll = useCallback((direction: 'left' | 'right') => {
    if (scrollRef.current) {
      ...
      const gap = 24;
      const scrollAmount = direction === 'left' ? -(clientWidth + gap) : (clientWidth + gap);
      scrollRef.current.scrollBy({ left: scrollAmount, behavior: 'smooth' });
    }
  }, []);

  return (
    <section id="testimonials" className="relative z-10 overflow-hidden" ref={ref}>
           ...
           {TestimonialsContent.map((item, idx) => (
              ...
              >
                 ...
                 <div className="flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 mt-auto relative z-10 w-full">
                    <div className={`w-10 h-10 md:w-14 md:h-14 shrink-0 rounded-full border-2 flex items-center justify-center bg-bg-base font-display font-bold text-sm md:text-lg ${ringColors[idx % ringColors.length]}`}>
                       {item.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div className="overflow-hidden w-full">
                       <div className="text-base md:text-lg font-display font-bold text-fg-bright truncate">{item.name}</div>
                       <div className="text-[11px] md:text-sm font-mono text-fg-dim truncate">{item.role} @ {item.company}</div>
                    </div>
                 </div>
              </motion.div>
           ))}
